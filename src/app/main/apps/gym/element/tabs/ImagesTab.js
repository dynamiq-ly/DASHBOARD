            ...
            render={({ field: { onChange } }) => {
              ...
              return images && typeof images === 'string' ? (
                <div
                  ...
                  role='button'
                  tabIndex={0}
                  className={clsx('productImageItem flex items-center justify-center relative w-128 h-128 rounded-16 mx-12 mb-24 overflow-hidden cursor-pointer outline-none shadow hover:shadow-lg')}>
                  <FuseSvgIcon className='productImageFeaturedStar'>heroicons-solid:star</FuseSvgIcon>

                  <img className='max-w-none w-auto h-full' src={`${process.env.REACT_APP_STORAGE_UTELLS}/gym/thumbnails/${images}`} alt='product' />
                </div>
              ) : (
                <div
                  onClick={() => onChange(images)}
