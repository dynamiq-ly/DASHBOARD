...
export const getMeasures = createAsyncThunk(
  'restaurantFacility/chefs/getMeasures',
  async (productId, { dispatch, getState }) => {
    const response = await axios.get(`/api/restaurant/chef?query=${productId}`)
...
export const removeMeasures = createAsyncThunk(
  'restaurantFacility/chefs',
  async (productIds, { dispatch, getState }) => {
    await axios.delete(`/api/restaurant/chef/${productIds}`, {
      data: productIds,
    })
    return productIds
  }
...
export const { selectAll: selectProducts, selectById: selectProductById } =
  measuresAdapter.getSelectors((state) => state.restaurantFacility.chefs)

const safetySlice = createSlice({
  name: 'restaurantFacility/chefs',
...
export const selectProductsSearchText = ({ restaurantFacility }) =>
  restaurantFacility.chefs.searchText
